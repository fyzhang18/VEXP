...
# create empty DF
yoo <- data.frame(RECORDING_SESSION_LABEL= character(),LEFT_GAZE_X= numeric(),LEFT_GAZE_Y= numeric(),LEFT_PUPIL_SIZE= numeric(),LEFT_IN_BLINK= numeric(),RIGHT_GAZE_X= numeric(),RIGHT_GAZE_Y= numeric(),RIGHT_PUPIL_SIZE= numeric(), RIGHT_IN_BLINK= numeric(), SAMPLE_INDEX= numeric(),SAMPLE_MESSAGE= character(),TIMESTAMP= numeric(),counterbalance= numeric(), imgseq= numeric(),side= numeric(),subID= numeric(), TRIAL_INDEX_RECODE= numeric(), TIMECODE_RECODE= numeric(),trial= numeric(),orig= numeric(),rleLength= numeric())
...
# Relabel raw pupil size as NA if the gaze coordinates are NA and it's not a blink
...
# Use the average pupil size during training trials, to calculate baseline. 
pooA <- subset(poo, TRIAL_INDEX_RECODE >0)
pooA$PUPIL_CORRECTED <- 999

subs <- unique(pooA$subID)
for (j in 1:length(subs)) {
  leftavg <- mean(pooA$RIGHT_PUPIL_SIZE[pooA$subID==subs[j] & pooA$phase==0 & pooA$trial==1 & pooA$side==1], na.rm = TRUE) #average pupil size for a subject, during training, when they're looking at a picture on the left side 
  rightavg <- mean(pooA$RIGHT_PUPIL_SIZE[pooA$subID==subs[j] & pooA$phase==0 & pooA$trial==1 & pooA$side==2], na.rm = TRUE) #average pupil size for a subject, during training, when they're looking at a picture on the right side 
  
  #percent pupil change from baseline for a subject, when they're looking to the left side during test is the (raw pupil size - left pupil average)/left pupil average
...
poo$PUPIL_CORRECTED[poo$phase==0] <- NA
```

For each trial, calculate percent trackloss (include NA and gazes that are off screen)
```{r}
poo$trackloss <- 99
...
for (j in 1:length(subs)) {
  ...
  print(subs[j])
  for (i in 1:length(trials)) {
  a <- length(which(is.na(poo$RIGHT_GAZE_X[poo$subID==subs[j] & poo$TRIAL_INDEX_RECODE==trials[i]])))  #total number of NA rows in a trial
  trackloss <- a/550 #550 because 550*4 = 2200ms
  poo$trackloss[poo$subID==subs[j] & poo$TRIAL_INDEX_RECODE==trials[i]] <- trackloss #add info to main DF
  }
  }
...
poo3$lookattarget[is.na(poo3$lookattarget)] <- 9

#double check it worked
blah <- subset(poo3, lookattarget==1 & side ==1)
...
blah <- subset(poo3, lookattarget==1 & side ==2)
ggplot(blah,aes(x=RIGHT_GAZE_X,y=RIGHT_GAZE_Y,color=factor(subID),fill=factor(subID)))+
  geom_point()+
  ggtitle("Right")+
  scale_x_continuous(limits=c(0,1280),breaks=seq(0,1280,200))+
...
blah <- subset(poo3, lookattarget==5)
ggplot(blah,aes(x=RIGHT_GAZE_X,y=RIGHT_GAZE_Y,color=factor(subID),fill=factor(subID)))+
  geom_point()+
  ggtitle("whatever")+
  scale_x_continuous(limits=c(0,1280),breaks=seq(0,1280,200))+
...
#calculate percentage looking at target during each trial
foo <- group_by(poo3, subID, TRIAL_INDEX_RECODE, trial) %>%
  summarise(
    percentlooking=length(which(lookattarget==1))/length(trial)
    )
```

Code every sample if they're looking at target (liberal)
```{r}
#(screen dimensions of screen are 1280 x 1024)
#might need to adjust these numbers to make the range larger for babies
leftx2 <- 500
rightx1 <- 780
poo3$lookattarget_lib <- 9

poo3 <- poo3 %>%
mutate(lookattarget_lib = ifelse(side ==1 & RIGHT_GAZE_X < leftx2, 1,
ifelse(side ==1 & RIGHT_GAZE_X > rightx1, 0, 
ifelse(side ==2 & RIGHT_GAZE_X > rightx1, 1, 
ifelse(side ==2 & RIGHT_GAZE_X < leftx2, 0,
ifelse(is.na(RIGHT_GAZE_X), 9, 5 ))))))

poo3$lookattarget_lib[is.na(poo3$lookattarget_lib)] <- 9

#double check it worked
blah <- subset(poo3, lookattarget_lib==1 & side ==1)
ggplot(blah,aes(x=RIGHT_GAZE_X,y=RIGHT_GAZE_Y,color=factor(subID),fill=factor(subID)))+
  geom_point()+
  ggtitle("Left")+
  scale_x_continuous(limits=c(0,1280),breaks=seq(0,1280,200))+
...
#calculate percentage looking at target during each trial
foo2 <- group_by(poo3, subID, TRIAL_INDEX_RECODE, trial) %>%
  summarise(
    percentlooking=length(which(lookattarget_lib==1))/length(trial)
    )

write.csv(poo, "VEXP_1-6_Infant_Preprocessed_AllSamples.csv", row.names=TRUE) #save to 
write.csv(poo3, "VEXP_1-6_Infant_Preprocessed_RemovedTrackloss.csv", row.names=TRUE) #save to computer
```
