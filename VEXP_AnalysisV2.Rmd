...
## Changes from V1:
- Data analysis using both PUPIL_CORRECTED and PUPIL_CORRECTED2
...
#THE TWO RESULT IN THE SAME % OF DATA, GOOD!
```
...
trialinfo <- group_by(test, subID, TRIAL_INDEX_RECODE) %>%
  summarize(percentmissing = unique(trackloss)
            )
...
orig.sample.a <- subset(orig.sample.a, trackloss < 0.5)
```
...
```{r}
# get test phase data
foo <- subset(orig.sample.a, phase==1)
...
foo <- group_by(poo, subID, TRIAL_INDEX_RECODE, trial, condition) %>%
  summarise(
    pupilsizeA=mean(PUPIL_CORRECTED, na.rm = TRUE),
...
foo2 <- group_by(foo, trial, condition) %>%
  summarise(
    meanPupilA=mean(pupilsizeA,na.rm = TRUE),
    sePupilA=sd(pupilsizeA, na.rm = TRUE)/sqrt(length(pupilsizeA)),
    meanPupilB=mean(pupilsizeB,na.rm = TRUE),
    sePupilB=sd(pupilsizeB, na.rm = TRUE)/sqrt(length(pupilsizeB))    
  )

# plot
limits <- aes(ymax = meanPupilA + sePupilA, ymin=meanPupilA - sePupilA)
dodge <- position_dodge(width=0.9)

ggplot(foo2,aes(x=factor(trial),y=meanPupilA,color=factor(condition),fill=factor(condition)))+
  theme_bw()+theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),axis.line = element_line(colour = "black"))+
  geom_bar(stat="identity",position=dodge)+
  ggtitle("Baseline A (All samples)")+
...
ggplot(foo2,aes(x=factor(trial),y=meanPupilB,color=factor(condition),fill=factor(condition)))+
  theme_bw()+theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),axis.line = element_line(colour = "black"))+
  geom_bar(stat="identity",position=dodge)+
  ggtitle("Baseline B (All samples)")+
  labs(x = "Trial", y = "Pupil change from baseline")+
  theme(plot.title = element_text(face="bold", size=20, hjust=0))+
  theme(axis.title = element_text(face="bold", size=20))+ 
  theme(axis.text.x  = element_text(size=20),axis.text.y  = element_text(size=20))+
...
ggplot(foo2,aes(x=factor(trial),y=meanPupilA,color=factor(condition),fill=factor(condition)))+
  theme_bw()+theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),axis.line = element_line(colour = "black"))+
  geom_bar(stat="identity",position=dodge)+
  ggtitle("Baseline A (Looking at target)")+
  labs(x = "Trial", y = "Pupil change from baseline")+
  theme(plot.title = element_text(face="bold", size=20, hjust=0))+
  theme(axis.title = element_text(face="bold", size=20))+ 
  theme(axis.text.x  = element_text(size=20),axis.text.y  = element_text(size=20))+
...
ggplot(foo2,aes(x=factor(trial),y=meanPupilB,color=factor(condition),fill=factor(condition)))+
  theme_bw()+theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),axis.line = element_line(colour = "black"))+
  geom_bar(stat="identity",position=dodge)+
  ggtitle("Baseline B (Looking at target)")+
  labs(x = "Trial", y = "Pupil change from baseline")+
  theme(plot.title = element_text(face="bold", size=20, hjust=0))+
  theme(axis.title = element_text(face="bold", size=20))+ 
  theme(axis.text.x  = element_text(size=20),axis.text.y  = element_text(size=20))+
...
# calculate mean pupil for each trial
foo <- group_by(poo, subID, TIMECODE_RECODE, condition) %>%
  summarise(
    pupilsizeA=mean(PUPIL_CORRECTED, na.rm = TRUE),
    pupilsizeB=mean(PUPIL_CORRECTED2, na.rm = TRUE)
  )
...
# collapse across subjects
foo2 <- group_by(foo, TIMECODE_RECODE, condition) %>%
  summarise(
    meanPupilA=mean(pupilsizeA,na.rm = TRUE),
    sePupilA=sd(pupilsizeA, na.rm = TRUE)/sqrt(length(pupilsizeA)),
    meanPupilB=mean(pupilsizeB,na.rm = TRUE),
...
ggplot(foo2,aes(x=TIMECODE_RECODE,y=meanPupilA,color=factor(condition),fill=factor(condition)))+
  geom_line(size=1)+
  theme_bw()+
...
ggplot(foo2,aes(x=TIMECODE_RECODE,y=meanPupilB,color=factor(condition),fill=factor(condition)))+
  geom_line(size=1)+
  theme_bw()+
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+
  ggtitle("Baseline B")+
...
poo <- subset(orig.sample.a, phase==1 & lookattarget==1)
...
totaltrials <- group_by(orig.sample.a, subID, phase) %>%
  summarise(
...
ggplot(foo3,aes(x=factor(condition),y=meanAEM,color=factor(AEM),fill=factor(AEM)))+
  theme_bw()+theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),axis.line = element_line(colour = "black"))+
  geom_bar(stat="identity",position=dodge)+
  labs(x = "Condition", y = "Eye movements during AEM window")+
  theme(plot.title = element_text(face="bold", size=20, hjust=0))+
  theme(axis.title = element_text(face="bold", size=20))+ 
  theme(axis.text.x  = element_text(size=20),axis.text.y  = element_text(size=20))+
  theme(legend.text=element_text(size=20),legend.title=element_text(size=20))+
...
ggplot(foo2,aes(x=factor(trial),y=meanLook,color=factor(condition),fill=factor(condition)))+
  theme_bw()+theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),axis.line = element_line(colour = "black"))+
  geom_bar(stat="identity",position=dodge)+
  ggtitle("Correct AEM")+
...
Speed of REM different between conditions (using mean)
```{r}
#1. CALCULATE BASELINE (using training phase) RT for REM 
...
# average baseline RT for each subject
aemrembaseline <- group_by(firstlook, subID) %>%
  summarise(
    baselinert = mean(TIMECODE_RECODE, na.rm = TRUE)
  )
...
rem_testphase <- group_by(firstlook, subID, condition) %>%
  summarise(
    testrt = mean(TIMECODE_RECODE, na.rm = TRUE)
  )
...
where are babies looking? wrong side, correct side, center
```{r}
# test phase & looks to screen
lookscode <- subset(orig.sample.a, phase==1 & lookattarget!=9) 
...
ggplot(epred,aes(x=TIMECODE_RECODE,y=looking,color=factor(group),fill=factor(group)))+
  ...
  scale_color_brewer(palette="Set1",name="Where are babies looking",breaks=c("1","2","3"),labels=c("Correct side", "Center","Wrong side"))+
  theme(plot.title = element_text(hjust = 0.5))+
...
```{r}
e2 <- ddply(lookscode,.(subID,condition,TIMECODE_RECODE),summarise,TargProp=mean(target,na.rm = TRUE),CentProp=mean(center,na.rm = TRUE),DistProp=mean(distractor,na.rm = TRUE))

#collapse across subjects
e31 <- ddply(e2,.(condition,TIMECODE_RECODE),summarise,
            looking=mean(TargProp,na.rm = TRUE),
            selooking=sd(TargProp, na.rm = TRUE)/sqrt(length(TargProp)))
e31$group <- 1
e32 <- ddply(e2,.(condition,TIMECODE_RECODE),summarise,
            looking=mean(CentProp,na.rm = TRUE),
            selooking=sd(CentProp, na.rm = TRUE)/sqrt(length(CentProp)))
e32$group <- 2
e33 <- ddply(e2,.(condition,TIMECODE_RECODE),summarise,
            looking=mean(DistProp,na.rm = TRUE),
            selooking=sd(DistProp, na.rm = TRUE)/sqrt(length(DistProp)))
e33$group <- 3
...
ggplot(efinal,aes(x=TIMECODE_RECODE,y=looking,color=factor(condition),fill=factor(condition)))+
  geom_line(size=2)+
  theme_bw()+theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+
